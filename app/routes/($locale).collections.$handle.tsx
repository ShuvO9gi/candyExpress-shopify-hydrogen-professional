...
function ProductsGrid({
  ...
  /* filter function */
  const handleClick = (tagNames: string[]) => {
    setSearchQuery('');
    const updatedCheckedItems = [...checkedItems];

    tagNames.forEach((tag_name) => {
      if (updatedCheckedItems.includes(tag_name)) {
        updatedCheckedItems.splice(updatedCheckedItems.indexOf(tag_name), 1);
      } else {
        updatedCheckedItems.push(tag_name);
      }
    });
    ...
    const sorted = products.filter((item: ProductItemFragment) =>
      updatedCheckedItems.some((tag_name) => item.tags.includes(tag_name)),
    );
  ...
  return (
    <div>
      ...
      <SearchInput
        searchCandy={searchCandy}
        searchFilter={searchFilter}
        setSearchFilter={setSearchFilter}
        setSearchCandy={setSearchCandy}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 justify-between gap-5 m-0 pt-[30px] pb-10 px-2.5">
        ...
        <div className="mb-[14px] md:mb-0 flex items-center gap-2.5">
          ...
          <div className="flex flex-col text-[#323232]">
            ...
            <p className="font-normal text-xs">Pakket med handsker og hårnet</p>
          </div>
        </div>
      </div>
    </div>
...
interface SearchInputProps {
  ...
  setSearchCandy: (value: boolean) => void;
...
interface FilterModalProps {
  ...
  setShowList: React.Dispatch<React.SetStateAction<ShowListState>>;
  checkedItems: string[];
  handleClick: (tag_name: string[]) => void;
}
                  ...
                  <div className="rounded-md shadow-[0_0_6px_rgba(0,0,0,0.07)]">
                    <div className="bg-white flex items-center justify-between px-3 py-[10px] border border-filterBorder rounded-md cursor-pointer">
                      ...
                      >
                        <img
                          ...
                          src={downArrow}
                          alt={
                            showList[groupName.group] ? 'Expand' : 'Collapse'
                          }
                          style={{
                    ...
                    <div
                      className={`overflow-y-scroll scrollbar-style ${
                        showList[groupName.group] ? 'max-h-[116px]' : 'max-h-0'
                        ...
                        {filteredTitles.map((groupTitle) => (
                          <li
                            className={`flex items-center justify-between px-3 py-[10px] first:pt-5 last:pb-5 cursor-pointer group ${
                              showList[groupName.group] ? 'group-active' : ''
                            }`}
...
function BottomSection({setSearchCandy, setSearchFilter}: BottomSectionProps) {
  return (
